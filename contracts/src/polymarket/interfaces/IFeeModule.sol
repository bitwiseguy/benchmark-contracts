...
/// @title IFeeModuleEE
/// @notice FeeModule Events and Errors
interface IFeeModuleEE {
    /// @notice Emitted when fees are withdrawn from the FeeModule
    event FeeWithdrawn(address indexed token, address indexed to, uint256 id, uint256 amount);

    /// @notice Emitted when fees are refunded to the order maker
    event FeeRefunded(bytes32 indexed orderHash, address indexed to, uint256 id, uint256 refund, uint256 feeAmount);
}

/// @title IFeeModule
/// @notice FeeModule interface
